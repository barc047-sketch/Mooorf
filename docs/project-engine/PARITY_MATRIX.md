...
| Table synchronization | Table edits the same `spaces` store projection as Canvas. | Automated: `TableView.performance.test.ts` protects canonical edit actions and bounded 300/500-row windows. Browser: an edit made in a virtualized Table row persisted to Canvas; end-of-list rows remained reachable. | Broader text-first editing, search, numeric-query, sort, and import synchronization remain PF1D.3 scope. | Do not create Table-local product state; the virtual slice must retain canonical store actions and truthful scroll geometry. |
