...
| Import | CSV/XLS/XLSX schedules validate before atomic central-store Add/Replace; warnings may proceed, errors block, and stale review/selection clears after apply. | Automated: `src/import/importCore.test.ts`, `src/import/tableFileWorkflow.test.ts`; Owner QA passed the file workflow. | Browser attachment mechanics remain Owner/manual evidence. | Preserve one file workflow and central-store commit boundary; review is ephemeral. |
...
| Table synchronization | Table edits the same `spaces` store projection as Canvas; approved 35/65 workspace, 70/30 Upload/Download cards, and full-width Search surround the bounded editor. | Automated: `TableView.performance.test.ts` protects canonical edit actions and bounded 300/500-row windows; `TableView.workspace.test.ts` covers the Table structure; import workflow tests cover 500-row import. Owner QA passed Canvas/store synchronization. | Numeric-query and sort are not claimed by this closeout. | Do not create Table-local product state; the virtual slice must retain canonical store actions and truthful scroll geometry. |
| PF1D workspace switching | Canvas spaces, camera, selection, renderer mode, open/minimized/moved widgets, Inspector shortcut, and Loader/readiness remain owned by their existing persistent instances while Table mounts lazily above them. Organism scheduling/drawing fully pauses during Table and resumes only after one prepared current-theme frame. | Automated: the PF1D.2 final command passed 28/28 with zero skips across scheduler, App/Organism lifecycle, Table performance, Organism wiring, and retained PF1D.1 contracts. Browser: 30/300/500-row entry passed; mounted rows stayed bounded, lazy requests remained 0→1→1, 12 rapid cycles were clean, and a 750ms Table probe observed zero Organism WebGL/2D draws. Owner PF1D.2 QA passed. | Remaining PF1D.3 progressive dense-row, expanded editing, search, numeric-query, sort, and import parity is separately gated; basic fixed-row windowing is complete. | Preserve one Canvas, one Table projection, one store, one Loader/readiness path, one Inspector listener, and permanently mounted chrome wrappers. Resume protection is App-owned and generation-safe; Classic remains unchanged. |
