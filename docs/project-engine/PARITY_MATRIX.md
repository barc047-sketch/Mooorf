...
| Runtime Status | One shell surface selects the highest-priority task and truthful notifications/FPS. | Automated: service behaviour in `pf1bContracts.test.ts`; source wiring/style contracts. | Mounted React integration for task/notification transitions. | Keep one activity service and renderer-fed FPS; no new loop/status owner. |
...
| Export | Snapshot is cloned once; Organism renders detached authored quality; composite owns padding/background. | Automated: `backgroundExportContracts.test.ts`, `exportCore.test.ts`, `svgExport.test.ts`; source wiring for detached Organism. | WebGL detached-render pixel/parity fixture and Owner visual output comparison. | Keep snapshot/provider/composite architecture; share only pure authored projection after characterization. |
