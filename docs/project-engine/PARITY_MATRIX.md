...
| Export | Snapshot is cloned once; Organism renders detached authored quality; composite owns padding/background. | Automated: `backgroundExportContracts.test.ts` characterizes authored presentation, Membrane, Cell Shadow, nuclei, and runtime-state exclusion; `exportCore.test.ts`, `svgExport.test.ts`; detached-Organism ownership wiring. | **E1 OPEN:** detached Membrane and SVG presentation overlays diverge under authored transforms/size variation. Add focused alignment contracts and Owner output comparison. | Keep snapshot/provider/composite architecture; share only pure authored projection after characterization; Classic SVG remains unchanged. |
...
**Outcome:** CSS ownership parity passed: 11 semantic blocks and 95 selector clauses moved with exact declaration/value/specificity/cascade parity; focused PF1B/PF1C contracts passed and Owner visual parity passed. E1 detached-export geometry parity remains OPEN; do not mark Organism detached export parity complete.
