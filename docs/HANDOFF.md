...
- **Dock:** tighter (30 px buttons, 47 px orb), saved-views popover replaced by a widget launcher, Random arrangement button added, palette popover gained "All palettes →" hand-off, void placeholder kept.
- **Palette:** 12 curated 10-step nucleus families (functional — ramp tints the category mapping via `getNucleusColor(..., nucleusPaletteId)`) + 12 organism palettes (9 solid functional via shader body/ground uniforms, 3 gradient blends staged with `ready: false`). New settings `nucleusPaletteId`/`organismPaletteId`, snapshot-persisted as optional fields so old snapshots keep loading.
...
- **Tokens/controls:** `--ease-out`, `--shadow-soft/float/deep`; custom range inputs (2 px hairline track, 11 px ink thumb, red hover halo, Firefox progress fill).
...
- **Deferred:** gradient/category organism blends (multi-color shader phase), custom palette slot, interface density, canvas label rename flow, negative nuclei.
