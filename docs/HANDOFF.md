...
- Correction 1 starts from reviewed head `1cc8ec0c7ab756bc6b091b87014da2c3e27383d2` and repairs only the five Owner-review groups. Current presets now arrange all renderer-visible entities with no selection or only the selected subset otherwise, preserve the arranged centroid and non-position data, and record one undo transaction.
- Batch Add Cells now uses Area/radius-aware golden-angle collision clearance around the viewport centre, multi-selects the new batch, and removes/restores the whole batch plus prior/new selection in one Undo/Redo transaction.
- Symbol pane geometry, filters, recents, favourites and placement controls now read canonical resources only; ephemeral `resourcesPreview` remains Canvas/export-preview input. Slider release commits that preview through one store owner, eliminating hover/focus remount loops.
- Presentation schema v5 gives Void Edge its own solid-by-default style/dash/gap/double-spacing values. Boundary and Void Edge reuse one current-six-style control and stroke projection across Classic, Organism overlay, raster capture and Classic SVG; Void geometry, Area, hit testing and clearance stay unchanged.
- Resource schema v3 adds `Auto Contrast | Custom` tint ownership and migrates legacy placements to Custom without changing their authored visible tint. All 133 approved symbols can target one Cell or Void and share apply/replace/remove, placement/backing, history, persistence and live/raster/Classic-SVG projection.
...
- Advanced Cell Shadow/Boundary/Core, Membrane Field/Motion/Edge and Void instruments route through existing project/default/store owners. Presentation schema v5 retains Membrane Edge softness/Core offsets and adds Void Edge style values; resource schema v3 owns complete Cell/Void symbol placement/backing, tint mode, recents and favourites.
...
- Symbol search/categories/recents/favourites/keyboard/hover preview/revert/apply/replace/remove and full placement/backing are live for Cells and Voids. History, project/import/saved-view persistence, Copy/Paste Style placement/backing, Classic/Organism Canvas, PNG/PDF and Classic SVG share the canonical owners.
...
## Current state
Prompt 03 execution: **Phases 1-4 COMPLETE**. **V4.5A COMPLETE** (docs, incl. reference patch). **V4.5B COMPLETE** (master graph domain layer). **V5 Readiness Audit COMPLETE**. **Phase 5 Table Sync COMPLETE**. **Phase 5.1 Table/Shell Polish COMPLETE**. **Phase 6A Organism Blob Foundation COMPLETE**. **Codex Workspace Setup COMPLETE**. **Phase 6B Visual Polish COMPLETE**. **Phase 6B.1 Blob Geometry Correction COMPLETE** (verified live). **Phase 6B.2 Morph Modes + Attachment Control COMPLETE**. **Phase 6C QA / Bugfix COMPLETE**. **Phase 6D Organism System Redesign + Morph Style Panel COMPLETE** (implementation only). **Phase 6E Organism Lab Shader Prototype COMPLETE** (implementation only). **V6F.0 Organism Production Integration Audit COMPLETE** (docs only). **V6F.0B Production Canvas UI / Control Architecture COMPLETE** (docs only). **V6F.0C Reference Folder Patch COMPLETE** (docs only). **V6F.0D GitHub-only Coding Workflow Setup COMPLETE**. **V6F.0F.3 GitHub Push / Doc Sync COMPLETE**. **V6F.1 Production Organism Canvas Integration COMPLETE**. **V6G QA / Stabilization COMPLETE**. **V6H Production Dock UI COMPLETE**. **V6H.1 Full Organism Control Surface COMPLETE**. **V6H.2 UI Command Architecture Cleanup + Resource Prep COMPLETE**. **V6H.3 Claude Premium UI Implementation COMPLETE**. **V6H.3Q Codex QA COMPLETE**. **V6H.4 Layout Presets / Colony Arrangement COMPLETE**. **V6H.4B Quick Add Cluster + Random Arrangement COMPLETE**. **V6I Palette + Category / Privacy Mapping COMPLETE**. **V6J Saved Views / Design Iterations COMPLETE**. **V6K Premium Visual System + Full Control Migration COMPLETE** (preview-verified). **V6L Multi-Color Organism Shader + Negative Nuclei COMPLETE**. **V6LQ Shader / Void QA COMPLETE** (no fixes needed). **V6M Codex Workflow OS COMPLETE**. **V6.5 Selection Arc + Canvas Rename COMPLETE**. **V6N Interface Density / Editorial Glass Refinement COMPLETE**. **V6N.1 Reference-Locked Premium Design System COMPLETE**. **V6N.2 Reference-Applied UI Polish COMPLETE**. **V6N.3 Premium Primitives + Moving Cell Border COMPLETE**. **V7.0 Spatial Intelligence System + Project Pulse Flagship COMPLETE** (preview-verified). **V7.0B Liquid Glass Shell Correction COMPLETE** (preview-verified). **V7.1 Spatial Intelligence Widget Family COMPLETE** (implementation/build; Terra live QA handoff next). Old canvas remains fallback.

## V7.1 — Spatial Intelligence Widget Family
- **Selectors:** four pure family APIs in `src/domain/stats/selectors.ts`, covered by malformed/empty/void edge tests. Widgets select the stable `spaces` array and derive via `useMemo`; no metric state enters Zustand.
- **Instruments:** Category Mix (top six + Other), Privacy Balance (public/shared/private/unknown + service context), Area Leaders (selectable valid top five), and Data Health (blocking numeric vs attention metadata signals).
- **Access:** one Stats rail launcher still opens Project Pulse; its compact Instruments popover opens all four through existing WidgetHost/openWidget ownership. Multiple panels may remain open with inherited liquid-glass depth.
- **Rules:** voids never inflate program/category/privacy totals; unknown strings normalize safely; no compliance thresholds, relationship guesses, table filters, camera focus, packages, chart framework, store/shader rewrite, or dashboard page.
- **Build/QA:** green; main chunk 733.00 kB, below the phase's roughly 750 kB lazy-load review line. Live QA covered all four launch paths/coexistence, add/add-five/void exclusion, table area/category/privacy sync, ranking selection, Data Health table handoff, Random neutrality, saved-view restore, day/night, 390 px, ORG/CLS, lab route, and zero console/NaN/Infinity output.
- **Next:** Terra performs a compact independent QA/fix pass. Relationship Health and Floor Summary remain deferred until live graph data exists.

## V7.0B — Liquid Glass Shell Correction
- **Material:** replaced near-opaque widget fill with shared day/night liquid-glass tokens (32px blur, 158% saturation, focused/background depth surfaces, WebKit support, unsupported-filter fallback). Computed day alphas are 0.58 front / 0.36 back.
- **Depth:** reused `openWidgets` order; `WidgetHost` passes one focused boolean and `WidgetFrame` exposes `data-depth`. Click focus, drag, minimize, restore, close, Escape, and mobile sheet stay in the existing frame.
- **Dock:** removed the global glass slab. Left quick controls and right utilities are independent `.glass` islands; center Add/Add-5 are solid near-black circles with white marks; Void stays hollow/subtractive.
- **Rail/density:** rail is 42px icon-only with ARIA labels and external glass tooltips. Shared headers, bodies, organism rows, and authored widths are tighter without removing controls.
- **Consistency:** Project Pulse, widgets, popovers, saved/palette surfaces, metadata, and edit overlays inherit the corrected shared tokens. Stats selectors and information hierarchy are unchanged.
- **Ponytail:** reused WidgetFrame/WidgetHost/openWidgets/dock/rail/shared controls/tokens. No dependencies, new components, state, selector changes, or duplicate glass system.
- **Build/QA:** build green (known chunk warning, main ~692.29 kB). Focused live QA covered stacked depth, dock/rail/actions, tooltips, Project Pulse, day/night, 390px, table sync, saved views, ORG/CLS, lab route, and console health.
- **Next:** Completed by V7.1; Terra compact QA/fixes is the active handoff.
...
- **Next:** Completed by V7.1. Relationship Health / Floor Summary stay future (need graph/floor runtime data).
