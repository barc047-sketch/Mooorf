...
| Boundary | `presentationDefaults.boundary` / sparse Cell override | Existing appearance preview/history | shared boundary projection; all six styles | project/Cell persistence; live/raster/SVG |
...
| Membrane Edge Softness | `presentationDefaults.membraneEdge.softness` | project-default preview/history | Classic stroke blur fallback / `uMembraneEdgeSoftness` | schema v4; PNG/PDF fallback |
| Void Fill / Edge | `presentationDefaults.void` / sparse Cell override | existing appearance preview/history | shared subtractive presentation projection | project/Cell persistence; live/raster/SVG |
...
| Symbol identity | `settings.resources.iconPlacements[].iconId` (one per target Cell) | `resourcesPreview`; `resources` history | shared `iconDrawing` adapter | resource schema v2; project/import/saved-view; PNG/PDF/SVG |
| Symbol placement/backing | same `IconPlacementSettings` record | `resourcesPreview`; one release history | shared `iconDrawing` adapter | resource schema v2; project/import/saved-view; PNG/PDF/SVG |
...
| Symbol recents/favourites | `settings.resources.iconRecents/iconFavourites` canonical IDs | `resources` history | Inspector filters | resource schema v2 |
