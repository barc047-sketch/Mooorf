...
assert.match(inspector, /Content[\s\S]*Appearance/i, "Inspector exposes only live Content and Appearance tabs");
assert.doesNotMatch(inspector, /Symbol library|Symbol browser/, "M1 does not expose a fake Symbol surface");
for (const id of ["cell-settings", "boundary-settings", "membrane-settings", "membrane-edge-settings", "core-settings", "void-settings"]) {
...
assert.match(details, /unsupported non-solid|solid fallback/, "Organism Boundary compatibility is surfaced truthfully");
...
assert.match(rail, /launcher\("inspector"/, "rail launches the production Inspector");
