...
equal(parseProjectEnvelope(JSON.stringify(project)).snapshot.settings.presentationDefaults.schemaVersion, 2, "project preserves presentation defaults");
...
equal(parsedSavedView.presentationDefaults?.schemaVersion, 2, "saved-view presentation defaults round-trip");
...
equal(parseConfigEnvelope(JSON.stringify(config)).settings.presentationDefaults.schemaVersion, 2, "config presentation defaults round-trip");
const legacyConfig = {
  ...config,
